...
이 과정에서 의존성 주입을 위한 Injecto를 구현하기 위해 특정 클래스가 가지고 있는 의존성을 파악하는 방법을 찾아야했다.  
...
> Reflect.defineMetadata 대신 자체 내부 전역변수를 사용하는 라이브러도 있다.

```ts
const injectablae: ClassDecorator = (target) => {
...
하지만 이번 포트스에서는 해당 내용을 다루지는 않았는데 다음과 같은 이유 때문이다.
