...
const a: Promise<null> = Promise.resolve(null).then(() =>
...
```typescript
function syncFunc<A, B>(a: A): B;
function asyncFunc<A, B>(a: A): Promise<B>;
```
...
monad 에 대한 정의보다 monad 를 통해 얻을 수 있는 장점에 대해 먼저 설명하였다.
